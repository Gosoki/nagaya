...
**设计与决策全部在 [`docs/SPEC.md`](docs/SPEC.md)（35 条已定决策 D1–D35）。动手前先读那个。**
...
**M1（记账 / 分摊 / 余额）与 M2（账单 / 结算 / 离线草稿）已完成。**
账单分「未出账 / 已出账 / 以前」三页，账目可按类型、分类、付款人筛选。
下一步 M3：设置面板、复制上月、模板、审计日志页、备份、部署。
