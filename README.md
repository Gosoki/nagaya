...
脚本会先验那份备份、**再把它的一份拷贝升到现在的表结构**（和开机同一套 Alembic
迁移，原件不动），然后把现在的账本整个挪到 `data/replaced-*/`（不删，恢复错了还能回头），
最后才换上去。所以加字段之前做的备份照样能恢复；升不上来的（比如更新的代码做的备份）
当场拦下，现有的账本不动。
...
能用了，日常记账 / 出账 / 结算这条主线是通的。

**成员**在「更多 → 设置 → 成员」里管：加人（给一个初始密码）、改入住日、标记搬走。
分摊按那笔账当天在籍的人算。**忘了密码**只能在服务器上重置：`tools.add_member --reset`
（密码只许本人改，界面上不给别人改的入口）。

**改表结构**：改完 `models.py` 要写一份迁移，开机会自动升级（升之前在 `data/` 里留一份
`before-migrate-*.db` 快照）。怎么写见 `backend/alembic/versions/README.md`；
漏写的话 `tests/test_migrations.py` 会红。
