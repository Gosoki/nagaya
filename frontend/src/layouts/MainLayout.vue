...
<template>
  <q-layout view="hHh lpR fFf">
    <!-- 顶栏固定在布局上，不放进页面里：账单两页之间切换时它不该跟着卸载重建，
         切页只换中间那块内容，上下两条都不动 -->
    <!-- **永远渲染，哪怕里头一样都没有。**
    ...
    <q-footer ref="footEl" class="app-bar footer-safe">
      ...
      <!-- 不用 q-route-tab：它的高亮跟着 vue-router 的 matched 链走，而编辑页、
           固定费那一屏这些路由不是分区的子路由，站在上面底栏一个 Tab 都不亮
           （账单曾经分三个平级路由时就踩过）。这里改成「路由 → 所属分区」自己算，点击自己导航 -->
      <q-tabs
