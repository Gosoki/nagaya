export default {
  ...
  entry: {
    ...
    needAmount: '多少钱？',
    amountLabel: '金额',
    conflictTitle: '这笔刚被别人改过',
    deleteConflict: '这笔刚被人改过（或者刚出了账），已经换成最新的了 —— 看一眼还要删的话再点一次',
    conflictCut: '这笔刚被出账归进了「{label}」—— 没人改过它。用你现在填的覆盖，改的就是那张已经发出去的账单（差额进下一张的上期结转）。要继续吗？',
    conflictBody: '别人刚改了：{changes}。用你现在填的覆盖吗？选「取消」就换成别人改完的样子',
    conflictOther: '分摊或备注',
    conflictDate: '日期',
  ...
  bill: {
    ...
    transferChanged: '数字刚变了（有人转了钱、或者又记了账），这次没记 —— 现在还差 {left}，看一眼再按',
    transferDoneElsewhere: '这一对现在已经不用转了（有人刚转过、或者方案变了），这次没记 —— 看看新的方案',
  ...
  profile: {
    title: '个人',
    photo: '头像', pickPhoto: '换张照片', removePhoto: '用回色圆', photoHint: '点头像换照片',
    displayName: '昵称', loginName: '登录名', lang: '语言',
    scheme: '外观', schemeAuto: '跟随系统', schemeLight: '浅色', schemeDark: '深色',
    schemeHint: '跟着账号走，换台设备登录也是这一套',
    themeColor: '主题色',
    themeColorHint: '导航、按钮和选中的颜色，跟着账号走',
    themeColors: { indigo: '藏青', blue: '海蓝', teal: '青绿', purple: '紫', rose: '玫红', amber: '琥珀', slate: '石板灰', graphite: '石墨' },
    ...
    passwordChangedRelogin: '密码已经改好了，但没能自动重新登录 —— 请用新密码登录（旧的已经不作数了）',
    logout: '退出登录',
  ...
  members: {
    ...
    notYet: '还没搬进来',
    leaveTitle: '{name} 搬走',
    leaveConfirm: '{date} 之后新记的账不再算 {name} 一份，已经记下的不变。TA 名下没结清的钱照样留在账单上，结清为止。',
    hint: '入住日、搬走日决定新记的账算不算这个人一份；已经记下的账不会重算。',
    add: '加一个人',
    displayName: '昵称',
    loginName: '登录名',
    password: '初始密码',
    joinedOn: '入住日',
    create: '加上',
    added: '加好了。把登录名「{login}」和初始密码告诉 {name}，TA 登录后在「个人」里把密码改掉。',
  },
  install: {
    title: '添加到主屏幕',
    done: '已添加',
    why: '从主屏打开是全屏的，没有地址栏，用起来就是一个 App。',
    already: '这台已经是从主屏打开的了。给别的手机加：用浏览器打开下面这个地址，再照着做。',
    address: '地址',
    addressLocal: 'localhost 只有这台电脑自己打得开。手机上用这台电脑的局域网 IP，端口 {port}',
    iphone: 'iPhone',
    android: '安卓',
    iosSafari1: '点 Safari 的分享按钮（方框加向上的箭头；新版 Safari 收在底部的「···」里）',
    iosSafari2: '往下滑，点「添加到主屏幕」',
    iosSafari3: '右上角点「添加」',
    iosOther1: '点浏览器的分享按钮',
    iosOther2: '点「添加到主屏幕」。找不到的话，换 Safari 打开这个地址再加',
    inApp1: '现在是 App 里自带的浏览器（比如 LINE），这里加不了主屏',
    inApp2: '点右下角的「···」或分享，选「用 Safari 打开」/「用浏览器打开」，再回到这里看怎么加',
    android1: '点 Chrome 右上角的「⋮」',
    android2: '点「添加到主屏幕」或「安装应用」，再点「添加」',
    noteLogin: 'iPhone 上从主屏打开之后要再登录一次：主屏的 App 和 Safari 的登录是分开的。',
    noteName: '名字和图标在加的那一刻就定了，之后再改，主屏上的不会变 —— 要改就先去「应用设置」改好再加。',
  },
  appearance: {
    title: '应用设置',
    name: '名字', namePlaceholder: '長屋 nagaya',
    icon: '图标', pick: '换个图标', removeIcon: '用回默认', applied: '改好了，正在刷新',
    iconHint: '点图标换一张。加到手机主屏后显示的就是它',
  },
  settings: {
  ...
  errors: {
    ...
    backup_clock_skew: '最新那份备份的时间在未来 —— 系统时间被调过，或者是从别处拷进来的。备份照常在做，确认一下服务器的时间对不对',
    ...
    member_dates_invalid: '搬走日（{left}）不能早于入住日（{joined}）',
    ...
    transfer_changed: '转账的数字刚变了，这次没记 —— 刷新一下再看',
