export default {
  app: { name: '長屋', tagline: 'シェアハウス家計簿' },
  common: {
    save: '保存', cancel: 'キャンセル', delete: '削除', edit: '編集', confirm: 'OK',
    back: '戻る', loading: '読み込み中…', yen: '{n}円', today: '今日', retry: '再試行',
    required: '必須', none: 'なし', all: 'すべて',
  },
  login: {
    title: 'ログイン', name: 'ユーザー名', password: 'パスワード', submit: '入る',
    failed: 'ユーザー名またはパスワードが違います',
  },
  nav: { add: '記帳', balance: '残高', entries: '明細', settings: '設定' },
  kind: { expense: '支出', income: '収入', settlement: '精算' },
  entry: {
    amount: '金額', category: 'カテゴリ', payer: '支払者', receiver: '受取者',
    to: '送金先', title: 'メモ（任意）', note: '備考', date: '日付',
    split: '分担', splitDetail: '分担を変更', saved: '記帳しました',
    saveAndNext: '保存して続けて記帳', save: '保存',
    emptyAmount: '金額を入力してください',
    coversPeriod: '請求対象期間', coversHint: '水道代など数か月分をまとめて払う場合に記入します',
  },
  split: {
    mode: '分担方法', ratio: '比率', exact: '金額指定',
    weight: '比率', adjustment: '調整額', share: '負担額',
    total: '合計', diff: '差額', balanced: 'ぴったり',
    notBalanced: '{n} 合いません。合わないと保存できません',
    hint: '比率 0 ＝ 参加しない。調整額は比率で割ったあとに加減します。',
  },
  monthly: {
    title: '今期の固定費',
    save: '{n} 件を保存',
    nothingChanged: '変更なし',
    recorded: '記録済み',
    willDelete: '削除されます',
    hintFrom: '{label} 参考',
    noHint: '記録なし',
    recordedOn: '記帳日 {date}（{label} に計上）',
    dateOutside: 'この日付は {label} に計上され、この請求書には載りません',
    coversPeriod: '請求対象期間',
    detail: '分担 / 請求対象期間',
    partialFail: '{n} 件保存できませんでした',
    addItem: '固定費を追加',
    addPlaceholder: '名称（例：NHK受信料）',
    added: '追加しました。来月から自動で表示されます',
    closedHint: 'この期間は締め済みです。編集するには解除してください',
  },
  bill: {
    title: '請求書', open: '請求書を出す', period: '対象期間',
    owed: '負担額', paid: '立替', prepaid: '前払い', carried: '前期繰越',
    toReceive: '受取', toPay: '支払', settled: '精算済み',
    total: '合計', income: '収入', dueBy: '{date} までに精算', unsettled: '未精算',
    covers: '{label} を含む',
    plan: '送金プラン（{n} 件）', planEmpty: '全員ぴったり。送金不要です',
    received: '受取済み', receivedHint: '{from} → {to} の送金を記帳します',
    copy: '請求書をコピー', copied: 'コピーしました。LINE に貼り付けてください',
    copyFallback: '下のテキストを長押しして選択・コピーしてください',
    close: '締める', closeConfirm: '締めるとこの期間の明細はロックされます。未精算の差額は次期に繰り越されます。',
    reopen: '締めを解除', closed: '締め済み', closedAt: '{date} 締め',
    noPeriod: 'まだ明細がありません',
  },
  draft: {
    pending: '未送信が {n} 件', submit: '送信', discard: '破棄',
    savedOffline: 'オフラインのため端末に保存しました',
  },
  balance: {
    title: '残高', owes: '{name} → {payer}', isOwed: '{name} 受取',
    settled: '精算済み', periodTotal: '{label} 合計', unsettled: '未締め',
    empty: 'まだ明細がありません',
  },
  errors: {
    network: 'サーバーに接続できません',
    unauthorized: 'ログインの有効期限が切れました',
    bad_sign: '金額の符号が違います（支出＝プラス、収入＝マイナス、精算＝プラス）',
    zero_amount: '金額は 0 にできません',
    not_integer: '金額は整数の円で入力してください',
    period_closed: '{label} は締め済みです。編集するには解除してください',
    version_conflict: 'この明細は今しがた更新されました。再読み込みしてください',
    sum_mismatch: '各自の合計と総額が {diff} 合いません',
    weights_all_zero: '分担する人がいません',
    negative_weight: '比率にマイナスは指定できません',
    unknown_member: '参加者以外が分担に含まれています',
    self_transfer: '自分自身には送金できません',
    missing_to_member: '送金先を選んでください',
    no_participants: 'この日に在籍メンバーがいません',
    unknown: 'エラー：{message}',
  },
}
