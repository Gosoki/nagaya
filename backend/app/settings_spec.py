...
SETTINGS_SPEC: dict[str, dict[str, Any]] = {
    ...
    "monthly_gap_days": {
        ...
        "note_ja": "前回の精算からこの日数に満たない場合、「固定費を含める」の初期値をオフにします。"
    ...
    "backup_path": {
        "type": "str",
        "default": "./backups",
        # 原来这里写的是「每日自动备份的存放目录」，而**备份功能一行代码都没有**。
        # 比没用更糟：它让人以为备份已经在跑了，于是不会自己去拷。
        # 真做的时候用 `VACUUM INTO ?`（对 WAL 安全、不用停服、产出单文件完整库）
        "note_zh": "备份目录。**备份功能还没做** —— 在它做出来之前请手动拷**整个 data/ 目录**，"
                   "只拷 nagaya.db 会得到一个空库（开着 WAL，数据在 -wal 文件里）。",
        "note_ja": "バックアップ先。**バックアップ機能は未実装**です。当面は data/ フォルダごと"
                   "手動でコピーしてください（WAL のため nagaya.db 単体では中身が空になります）。",
    },
}
